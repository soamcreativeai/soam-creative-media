...
## 2026-08-07（定時公開が本物のcronで初めて最後まで成功）

- 発見: 直前の2つの修正を反映した直後の実際の定時実行（14:51/15:01 JST）が、「関連記事3件以上」の公開契約チェックで2回連続失敗していた。AIが返す関連記事候補が2件しかない時、不足分を補う処理が無かったため。
- 修正: 候補が3件未満の時は記事一覧から不足分を自動補完するよう変更（[`130f945`](https://github.com/soamcreativeai/soam-creative-media/commit/130f945)）。
- 検証: dry-runなしの本物の公開経路を手動起動し、article-88の生成・検査・commit・push・Cloudflare Pages反映・本番URL表示まで完走を確認（[run 31150993899](https://github.com/soamcreativeai/soam-creative-media/actions/runs/31150993899)）。今回の一連の修正で、定時公開が最初から最後まで通しで成功したのはこれが初めて。
- 未確認: 次回以降の本物のcronトリガー（次は本日20:02 JST）での継続的な成功。
...
- 変更: 朝・昼・晩の主起動に対し、各17分後に保険起動を追加。保険は公開IDを最初に照合し、既に記録済みならOpenAI、Cloudflare、記事・一覧・sitemapの更新をせず終了する。未記録の時だけ通常の1本を生成する。
- 追加: 公開契約検査とslot判定検査を追加。定時起動の一本化、保守用workflowの非定時化、重複時の有料生成前停止、1枠1回、公開までの全工程を検査対象に固定。
- 検証: 記録済み枠をAI設定なしで実行して no-op を確認。slot判定、公開契約、fixtureによるarticle-88相当の生成→公開→SEO→戦略→Link、既存87記事検査、Pages buildがPASS。
- 本番確認: 次回の通常定時実行で確認予定。手動生成・手動公開は行わない。

## 2026-08-06（出典リンクを広告ボタンから分離）

- 原因: 定時生成したアフィリエイト記事の出典リンクが、広告カード用の公式情報リンクと同じ計測種別だった。広告ボタンを一本化する検査が出典リンクを重複と判定し、公開直前に停止していた。
- 変更: 出典リンクを専用の `source_reference_click` として扱い、広告クリックと混同しないようにした。未管理の外部URLを拒否する検査は維持し、回帰検査を追加。
- 検証: 87記事の全検査と、隔離環境で生成したarticle-88相当の生成→公開→SEO→戦略→Link検査がPASS。
- 追加: 同じ生成→公開→SEO→戦略→Link検査を、生成APIより前に毎回実行する事前検査としてworkflowへ追加。検査失敗時は記事生成APIを呼ばない。
- 共有反映: `6075892`、事前検査 `565c6f8`。次回定時の実OpenAI生成で確認予定。手動生成・手動公開は実施していない。
...
## 2026-08-02（定時記事作成・公開を一本化）

- 発見: `Publish scheduled articles` は手動起動だけで、定時記事作成・公開の責任は `Generate and publish SOAM Media article` に寄せるべき状態だった。
- 変更: JST 07:02 / 12:02 / 20:02の1本の定時処理だけで、記事作成・品質確認・公開・Cloudflare Pages反映・カスタムドメイン確認まで実行する。公開専用workflowの定時起動は設けない。
