...
## 2026-08-07 定時公開の3件目の隠れたバグを修正（**手動起動で**完走を確認・無人での完走は未確認）

- 前の2つの修正（GITHUB_OUTPUT形式エラー・「向いている人／向いていない人」要件の欠落）を本番反映した直後、実際の定時実行(14:51 JST・15:01 JSTの保険起動)が**別の理由で2回連続失敗**した。原因: 公開契約が「関連記事は3件以上」を必須にしているのに、AIが返す関連記事候補が2件しかない時に不足分を補う処理が無かった。生成・品質チェックには通っても、この最後の検査で毎回止まっていた。
- 修正: AIの候補が3件未満の時は、記事の候補一覧から不足分を自動で補うようにした（commit `130f945`）。
- 検証: 本物の公開経路（dry-runなし・**手動起動**）で実行し、[article-88](https://media.soam-creative.com/articles/article-88.html)が実際に生成・検査・commit・push・Cloudflare Pages反映・本番URLでの表示まで完走することを確認した（[run 31150993899](https://github.com/soamcreativeai/soam-creative-media/actions/runs/31150993899)）。**【2026-08-07 監査で訂正】これは手動起動での成功であり、無人の定時実行(cron)での完走ではない。** 無人で最後まで通ったのは2026-08-01 14:52 JST(run 30686644546)が最後。scheduleトリガーは全38件中 失敗34・成功4で、成功4件のうち直近2件(8/7 20:43・20:55)は生成ステップがskippedの空振り。8/7夜枠は14:33の手動テストが同じ枠を先に消費したため検証できなかった。**次の検証機会は8/8朝の枠。**
- 未確認: 無人のcronトリガーでの完走。⚠️**無人実行を検証したい日は、同じ枠で手動テストをしない**（手動テストが枠を消費し重複防止でskipされるため）。
