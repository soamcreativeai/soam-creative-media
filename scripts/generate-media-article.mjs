...
const responseRequest = async (input) => {
  ...
  const systemPrompt = 'SOAM Mediaの編集者として、事実・実績・料金・体験を創作せず、日本語の構造化記事データだけを返す。HTML、URL、広告リンクは返さない。未提供機能を提供中と書かない。'
    ...
    + '(4) 同じ内容の文をそのまま繰り返さない。'
    + '(5) 本文のどこかに「向いている人」「向いていない人」という語をそのまま使い、両方の判断基準を明記する。';
