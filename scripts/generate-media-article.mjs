...
const previewPage = (entry, bodyHtml, date) => {
  ...
  return `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="description" content="${esc(entry.article.metaDescription)}"><title>${esc(entry.article.title)} | SOAM CREATIVE</title><link rel="canonical" href="${canonicalUrl(`articles/${entry.article.slug}.html`)}"><script type="application/ld+json">${jsonForScript(data)}</script><link rel="stylesheet" href="../../style.css"></head><body><main class="article-body"><div class="article-container"><p class="article-category">${esc(entry.article.categoryLabel)}</p><h1>${esc(entry.article.title)}</h1><p class="article-date">${date}</p>${bodyHtml}</div></main></body></html>`;
};
...
const main = async () => {
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    errors = qualityErrors({ article: normalized, styleGuide: catalog.styleGuide, expectedCategory: choice.theme.category, expectedTitlePool: manifest.map((article) => article.title), expectedTextPool: manifest.map((article) => `${article.title} ${article.excerpt} ${(article.tags || []).join(' ')}`), expectedRelatedIds: manifest.map((article) => article.slug), offers });
  ...
  const scheduledAt = `${date}T${slots[slot]}:00+09:00`; const relatedArticles = normalized.relatedArticleIds.length ? normalized.relatedArticleIds : manifest.filter((article) => article.category === choice.theme.category).slice(-3).map((article) => article.slug);
