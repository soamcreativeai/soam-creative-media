...
const articleSchema = {
  ...
  properties: {
    title: { type: 'string' }, meta_description: { type: 'string' }, excerpt: { type: 'string' }, primary_keyword: { type: 'string' }, secondary_keywords: { type: 'array', items: { type: 'string' } }, search_intent: { type: 'string' }, target_reader: { type: 'string' }, pillar: { type: 'string' }, introduction: { type: 'string' }, problem_structure: { type: 'array', minItems: 3, items: { type: 'string' } }, decision_criteria: { type: 'array', minItems: 3, items: { type: 'object', additionalProperties: false, required: ['criterion', 'how_to_judge'], properties: { criterion: { type: 'string' }, how_to_judge: { type: 'string' } } } }, steps: { type: 'array', minItems: 3, items: { type: 'string' } }, checklist: { type: 'array', minItems: 3, items: { type: 'string' } }, conclusion: { type: 'string' }, source_ids: { type: 'array', minItems: 1, items: { type: 'string' } }, related_article_ids: { type: 'array', items: { type: 'string' } }, affiliate_recommendations: { type: 'array', items: { type: 'object', additionalProperties: false, required: ['offer_id', 'reason'], properties: { offer_id: { type: 'string' }, reason: { type: 'string' } } } }, sections: { type: 'array', minItems: 9, items: { type: 'object', additionalProperties: false, required: ['heading', 'paragraphs', 'subsections'], properties: { heading: { type: 'string' }, paragraphs: { type: 'array', minItems: 2, items: { type: 'string' } }, subsections: { type: 'array', items: { type: 'object', additionalProperties: false, required: ['heading', 'paragraphs'], properties: { heading: { type: 'string' }, paragraphs: { type: 'array', minItems: 1, items: { type: 'string' } } } } } } } }
  }
};
...
const main = async () => {
  ...
  for (let themeIndex = 0; themeIndex < maxThemeCandidates; themeIndex += 1) {
    ...
    for (let attempt = 1; attempt <= maxRevisions; attempt += 1) {
      ...
      const sections = (generated.sections || []).map((section) => String(section.heading || '').trim() === 'まとめ' ? { ...section, heading: '最後に確認したいこと' } : section);
