...
assert.equal(qualityErrors({ article: { ...article, bodyHtml: `${article.bodyHtml}<a href="https://example.com/source" data-track-event="source_reference_click">出典</a>` }, styleGuide: catalog.styleGuide, expectedTitlePool: manifest.map((item) => item.title), offers: [] }).length, 0, '出典リンクは広告用ボタンとは別の計測として許可する');
assert.match(qualityErrors({ article: { ...article, bodyHtml: `${article.bodyHtml}<a href="https://example.com/unmanaged">未管理</a>` }, styleGuide: catalog.styleGuide, expectedTitlePool: manifest.map((item) => item.title), offers: [] }).join('\n'), /未管理の外部URL/, '未管理の外部リンクは拒否する');
